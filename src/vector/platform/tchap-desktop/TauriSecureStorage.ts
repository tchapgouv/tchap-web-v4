...
export class TauriSecureStorage {
    ...
    public constructor(ipcManager: TauriIPCManager) {
        this.ipc = ipcManager;
    }
    ...
    public async getItem(key: string): Promise<any> {
        try {
            const item = await this.ipc.call("get_password", { user: key});
            return item
        } catch(err) {
    ...
    public async createItem(key: string, value: any): Promise<any> {
        try {
            await this.ipc.call("set_password", { user: key, password: value});
        } catch(err) {
    ...
    public async removeItem(key: string): Promise<any> {
        try {
            await this.ipc.call("delete_password", { user: key});
        } catch(err) {
