...
describe("Provides utils method to get room type and state", () => {
    ...
    it("returns room type depending on encryption and access rule", (done) => {
        expect(TchapRoomUtils.getTchapRoomTypeInternal(true, TchapRoomAccessRule.Restricted))
            .toStrictEqual(TchapRoomType.Private);
        expect(TchapRoomUtils.getTchapRoomTypeInternal(true, TchapRoomAccessRule.Unrestricted))
            .toStrictEqual(TchapRoomType.External);
